...
import {
  Users, Building2Icon, X, Edit2, Trash2
} from 'lucide-react';
...
const TrainerClasses = () => {
  ...
  const [form, setForm] = useState({ name: "", schedule_time: "", capacity: 20 });
  ...
  setLoading(true);
  try {
    // Fetch classes of logged-in trainer using user ID
    const loggedInUserId = user?.id;
    const response = await api.get(`/trainer/classes/trainer/${loggedInUserId}`);
  ...
  const openAddModal = () => {
    ...
    setForm({ name: '', schedule_time: '', capacity: 20 });
    setIsAddModalOpen(true);
  };
  ...
  const handleAddSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post("/admin/classes", form); // backend will use JWT for ownership
    ...
    if (classes.length === 0) {
      return (
        <div className="border-4 border-black p-6 bg-yellow-100 shadow-[4px_4px_0_0_#000]">
          <p className="font-bold text-lg">No classes found under your trainer account.</p>
        </div>
      );
    }
    return (
        ...
        {classes.map((cls) => (
          ...
          >
            ...
            <div className="space-y-2 text-sm pt-2">
              <p>
                <strong className="font-bold text-gray-800">Time:</strong> {new Date(cls.schedule_time).toLocaleString('en-US', {
                  month: 'short',
                  day: 'numeric',
                  hour: '2-digit',
                  minute: '2-digit'
        ...
        <div className="bg-white border-4 border-black shadow-[10px_10px_0_0_#000] w-full max-w-lg p-6 relative">
          ...
          <form onSubmit={submitHandler}>
            ...
            <div className="flex justify-end pt-4 border-t border-gray-200">
              <button type="submit"
                className="bg-blue-500 text-white border-4 border-black px-6 py-3 font-bold shadow-[4px_4px_0_0_#000] hover:bg-blue-600">
                {buttonText}
              </button>
              {isEditing && (
                <button type="button"
                  onClick={() => { setClassToDelete(editingClass); setIsAddModalOpen(false); }}
                  className="ml-2 bg-red-500 text-white border-4 border-black px-4 py-2 font-bold shadow-[4px_4px_0_0_#000] hover:bg-red-600">
